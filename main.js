...
// ── Multi-series line chart (one line per category over time) ────────────────

function buildMultiLineChart(title, datasets, fullLabels = false) {
  if (!datasets?.length) return "<p style='padding:8px'>No data</p>";

  const PALETTE   = ["#00b4ff", "#ff6b35", "#7ed321", "#f8e71c", "#bd10e0", "#50e3c2"];
  const PAD_L     = fullLabels ? 52 : 44;
  const PAD_R     = fullLabels ? 20 : 16;
  const PAD_TOP   = fullLabels ? 42 : 36;
  const PLOT_W    = fullLabels ? 480 : 340;
  const PLOT_H    = fullLabels ? 160 : 130;
  const LEG_ROW_H = fullLabels ? 22 : 18;
  const LEG_ROWS  = Math.ceil(datasets.length / 2);
  const PAD_BOT   = (fullLabels ? 54 : 44) + LEG_ROWS * LEG_ROW_H;
  const FS_TITLE  = fullLabels ? "14" : "12.5";
  const FS_LABEL  = fullLabels ? "10.5" : "9";
  const FS_VAL    = fullLabels ? "10"   : "9";
  const FS_LEG    = fullLabels ? "10.5" : "9";
  const TITLE_Y   = fullLabels ? 27 : 23;
  const MAX_LBL   = fullLabels ? 14 : 9;

  const labels = datasets[0]?.chartData.map(d => d.label) ?? [];
  const n = labels.length;
  if (!n) return "<p style='padding:8px'>No data</p>";

  const vbW    = PAD_L + PLOT_W + PAD_R;
  const vbH    = PAD_TOP + PLOT_H + PAD_BOT;
  const maxVal = Math.max(...datasets.flatMap(ds => ds.chartData.map(d => d.value)), 1);
  const xs     = labels.map((_, i) => PAD_L + (n === 1 ? PLOT_W / 2 : (i / (n - 1)) * PLOT_W));

  const grid = [0, 0.33, 0.67, 1].map(pct => {
    const y = PAD_TOP + PLOT_H - pct * PLOT_H;
    return `
      <line x1="${PAD_L}" y1="${y}" x2="${PAD_L + PLOT_W}" y2="${y}" stroke="#2a2a40" stroke-width="0.8"/>
      <text x="${PAD_L - 5}" y="${y + 4}" text-anchor="end" font-size="${FS_VAL}" fill="#666" font-family="sans-serif">${Math.round(pct * maxVal)}</text>`;
  }).join("");

  const lines = datasets.map((ds, di) => {
    const color = PALETTE[di % PALETTE.length];
    const ys    = ds.chartData.map(d => PAD_TOP + PLOT_H - (d.value / maxVal) * PLOT_H);
    const path  = `M ${xs[0]},${ys[0]} ` + xs.slice(1).map((x, i) => `L ${x},${ys[i + 1]}`).join(" ");
    const dots  = xs.map((x, i) =>
      `<circle cx="${x}" cy="${ys[i]}" r="${fullLabels ? 3.5 : 3}" fill="${color}" stroke="#1a1a2e" stroke-width="1.5"/>`
    ).join("");
    return `<path d="${path}" fill="none" stroke="${color}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" opacity="0.9"/>${dots}`;
  }).join("");

  const xLabels = xs.map((x, i) => {
    const lbl = labels[i].length > MAX_LBL ? labels[i].slice(0, MAX_LBL - 1) + "…" : labels[i];
    const ly  = PAD_TOP + PLOT_H + 18;
    return `<text x="${x}" y="${ly}" text-anchor="end" font-size="${FS_LABEL}" fill="#ccc" font-family="Cairo,sans-serif" transform="rotate(-35,${x},${ly})">${lbl}</text>`;
  }).join("");

  const legendY0 = PAD_TOP + PLOT_H + (fullLabels ? 46 : 38);
  const legend = datasets.map((ds, di) => {
    const color = PALETTE[di % PALETTE.length];
    const x     = PAD_L + (di % 2) * (PLOT_W / 2);
    const y     = legendY0 + Math.floor(di / 2) * LEG_ROW_H;
    const name  = ds.label.length > 18 ? ds.label.slice(0, 17) + "…" : ds.label;
    return `<rect x="${x}" y="${y - 8}" width="10" height="10" rx="2" fill="${color}"/>
      <text x="${x + 14}" y="${y}" font-size="${FS_LEG}" fill="#ccc" font-family="Cairo,sans-serif">${name}</text>`;
  }).join("");
...
function handleSlottableRequest(e) {
  ...
  try {
    ...
    const isMultiLine  = !isComparison && p.chartType === "line" && Array.isArray(p.datasets);
    const isLine       = !isComparison && !isMultiLine && p.chartType === "line";

    const svgHtml = isComparison
      ? buildComparisonChart(p.title, p.datasets)
      : isMultiLine
        ? buildMultiLineChart(p.title, p.datasets)
        : isLine
    ...
    const svgHtmlFull = isComparison
      ...
      : isMultiLine
        ? buildMultiLineChart(p.title, p.datasets, true)
        : isLine
